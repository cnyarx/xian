...
前面的 [[Weyl 反常]] 与 [[反常消除]] 已经说明：经典对称性、量子测度、正规化和完整一致性条件必须分开检查。接下来需要进入弦世界面真正使用的计算语言。

二维 Euclidean 场论把以下内容连接起来：

1. [[Wick 旋转]] 把 Lorentz 振荡权重变为 Euclidean 衰减权重；
2. [[路径积分]] 和 [[生成泛函]] 定义关联函数；
3. [[复分析]] 把两个实坐标组织成 $z,\bar z$；
4. [[Green 函数]] 给出二维自由场的对数二点函数；
5. [[能动张量]]、Weyl 变换和二维共形结构组织守恒律；
6. [[共形映射]] 的指数映射把圆柱 Euclidean 时间变成平面半径；
7. [[黎曼曲面]] 提供高亏格弦世界面的全局几何。

本页只建立共同约定和计算入口。复坐标下的系统全纯分解由 [[复坐标与全纯分解]] 展开，量子自由场计算由 [[二维自由玻色子]] 展开；一般 OPE、共形权、径向量子化、态—算符对应以及 Virasoro 代数留给后续独立节点。
...
本页沿用 [[Weyl 反常]] 的 Euclidean 变分约定。经典能动张量定义为
...
然而“无质量”不是量子共形不变的充分条件。耦合常数的 beta 函数、正规化、改进能动张量和 [[Weyl 反常]] 都可能改变量子结论。
...
并不保证量子期望值仍为零。在非手征二维 CFT 和 [[Weyl 反常]] 所采用的 Euclidean 约定下：
...
15. 后继 [[复坐标与全纯分解]] 系统推导向量、一形式、张量分量和局部全纯分解，[[二维自由玻色子]] 则在同一约定下计算量子二点函数与能动张量短距离奇异项。
...
- [[反常消除]] 是上一路线节点，说明进入世界面场论前必须区分不同一致性条件；
- [[Wick 旋转]] 连接 Lorentz 权重 $e^{iS_M}$ 与 Euclidean 权重 $e^{-S_E}$；
- [[路径积分]] 和 [[生成泛函]] 定义 Euclidean 配分函数与关联函数；
- [[复分析]] 提供 $z,\bar z$、Wirtinger 导数和全纯函数语言；
- [[度量]] 定义二维面积元、Laplace–Beltrami 算符和 Weyl 变换；
- [[Green 函数]] 解释自由场二点函数为何是动力学算符的逆；
- [[Dirac delta]] 给出对数 Green 函数的分布归一化；
- [[传播子]] 连接 Euclidean 二点函数、极点处方和 Lorentz 传播；
- [[能动张量]] 定义度量变分、守恒流和迹；
- [[Weyl 反常]] 说明经典迹为零为何可能在量子层面失效；
- [[共形映射]] 给出 $z=e^w$ 和二维局部角度保持变换；
- [[黎曼曲面]] 把局部复坐标推广到弦世界面的全局几何；
- [[二维自由玻色子]] 继承本页作用量、Green 函数和 timelike 边界，进一步计算导数收缩、正规序和中心荷；
- [[紫外发散与红外发散]] 解释二维无质量标量的红外尺度和调节器；
- [[弦理论完整学习路线图]] 把本页放在反常消除与复坐标、自由场、CFT 结构之间；
