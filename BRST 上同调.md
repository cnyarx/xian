...
[[上同调]] 说明：若一个分次空间上有平方为零的微分，就可以研究“闭对象模去恰当对象”。BRST 理论把这一骨架应用到 ghost number 分次的态或算符空间。
...
协变量子化先得到一个包含非物理规范模式和鬼自由度的扩展候选空间。这个空间可能具有不定内积，不能直接当作满足概率解释的物理 [[Hilbert 空间]]。
...
下一节点 [[量子反常]] 将讨论路径积分测度、Wess-Zumino 一致性条件和反常消除。
...
- [[BRST 对称性]] 提供幂零荷 $Q$、ghost number 和物理态条件；
- [[上同调]] 提供闭对象模去恰当对象的一般数学结构；
- [[Faddeev-Popov 鬼场]] 提供鬼自由度和 ghost number 的路径积分来源；
- [[Hilbert 空间]] 帮助区分不定内积候选空间与最终物理态空间；
- [[场的正则量子化]] 和 [[Fock 空间]] 提供候选振子态空间；
- [[Yang-Mills 理论]] 提供目标时空内部规范群的主要应用；
- [[Maxwell 场量子化]] 提供 Abelian 物理极化的原型；
- [[弦理论完整学习路线图]] 将本节点放在 BRST 对称性与量子反常之间。
- [[量子反常]] 把 BRST 上同调作为局域一致反常候选的分类工具，并展开完整的反常总览、消除和边界。
