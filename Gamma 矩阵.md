...
- [[Clifford 代数]] 给出 Gamma 矩阵必须满足的抽象生成关系；
- [[矩阵]] 提供相似变换、迹、转置和 Hermitian 共轭；
- [[张量积]] 可递归构造更高维 Gamma 矩阵；
- [[SO 群]] 的 Spin 覆盖在旋量空间上的生成元由 Gamma 对易子实现；
- [[旋量]] 是 Gamma 矩阵作用的向量；
- [[Dirac 场]] 使用 Gamma 矩阵构造相对论一阶波动方程；
- [[Ramond 零模]] 经过归一化后可由 Gamma 矩阵表示；
- [[时空旋量]] 在十维中可施加 Majorana 与 Weyl 条件；
- [[超弦临界维数]] 的谱与时空超对称分析需要十维旋量结构。
