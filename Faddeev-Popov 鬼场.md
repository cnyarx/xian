...
[[Faddeev-Popov 行列式]] 已给出规范固定后路径积分中的行列式 $\Delta_{\mathrm{FP}}[A]=\det M[A]$。但功能行列式是非局域对象，难以直接用于微扰展开和 Feynman 规则推导。
...
功能行列式 $\det M[A]$ 难以直接处理。但 [[Grassmann 变量]] 已说明：复 Grassmann 场的 Gaussian 积分产生行列式。
...
- [[BRST 上同调]] 给出物理态空间，排除 BRST-exact 的非物理态。

完整 BRST 变换、辅助场和幂零结构见下一节点 [[BRST 对称性]]；上同调类的系统分类再由后续独立页面展开。
...
- [[Faddeev-Popov 行列式]] 给出鬼场的起源和功能行列式；
- [[规范固定]] 提供规范轨道、切片和 FP 插入恒等式的背景；
- [[Yang-Mills 理论]] 提供非 Abelian 规范群、结构常数和协变导数约定；
- [[Grassmann 变量]] 提供 Grassmann 积分表示行列式的数学基础；
- [[Maxwell 场量子化]] 提供 Abelian 原型和鬼场解耦的对比；
- [[圈展开]] 说明鬼场在圈图中的统计符号和作用；
- [[弦理论完整学习路线图]] 将本节点放在 FP 行列式之后、BRST 对称性之前。
