...
[[规范固定]] 会产生 [[Faddeev-Popov 行列式]]。可用 Grassmann 奇标量场 $c,\bar c$ 表示该行列式：
...
- [[费米子]] 的产生算符和场使用反对易结构；
- [[外代数]] 与 Grassmann 代数共享反对称乘法；
- [[微分形式]] 使用楔积形成分次反交换代数；
- [[费米场路径积分]] 用 Berezin 积分产生行列式；
- [[二维自由费米子]] 用 Grassmann 奇场、换序负号和一阶动力学算符构造二维自由 CFT；
- [[Faddeev-Popov 鬼场]] 用 Grassmann 场表示规范固定行列式；
- [[BRST 对称性]] 依赖鬼数和 Grassmann 奇生成元；
- [[世界面费米子]] 在 RNS 路径积分中取 Grassmann 奇值；
- [[RNS 超弦理论]] 同时使用世界面费米场和鬼系统。
