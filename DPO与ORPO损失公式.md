...
tags: [公式, 对齐, 偏好优化, 大模型, 学习教程]
prerequisites: ["[[似然最大似然与负对数似然]]", "[[策略奖励优势与基线]]", "[[Softmax稳定计算与温度公式]]", "[[激活函数公式]]"]
---
# DPO 与 ORPO 损失公式
...
其中 SFT 项通常使用序列对数概率的均值（长度归一化）：

$$
\mathcal L_{SFT}=-\frac{1}{|y_w|}\sum_{t=1}^{|y_w|}\log P_\theta(y_{w,t}\mid x,y_{w,<t})
$$

接下来定义同一口径下的序列概率，用于构造 odds：

$$
P_\theta(y\mid x)=\exp\left(\frac{1}{|y|}\sum_{t=1}^{|y|}\log P_\theta(y_t\mid x,y_{<t})\right)
$$
...
ORPO 不需要单独的参考模型；$\lambda$ 控制偏好项相对 SFT 项的权重。实际实现中的长度归一化、概率聚合和数值稳定细节应以论文或代码为准。

## 5. 边界

- DPO 不需要显式训练奖励模型，但仍需要偏好数据和参考模型；
- DPO 的推导建立在特定 KL 正则化奖励建模关系上，不等于任意二分类损失；
- ORPO 的序列概率使用长度归一化的几何平均，与直接连乘的原始概率不等价；实现细节应以论文或代码为准；
