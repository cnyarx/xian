...
- [[Lie 群]] 提供连续矩阵群的总体框架；
- [[Lie 代数]] 描述 $SO(n)$ 的无穷小生成元；
- [[矩阵]] 提供转置、逆矩阵、乘法和行列式语言；
- [[内积空间]] 解释正交变换为何保持长度和角度；
- [[SU(2) 与 SO(3) 的双覆盖]] 详细解释三维旋转与半整数自旋；
- [[Clifford 代数]] 用于构造 $Spin(n)$；
- [[Gamma 矩阵]] 给出 Clifford 代数的矩阵表示；
- [[旋量]] 是旋转覆盖群的表示对象；
- [[角动量]] 是三维旋转的量子生成元；
- [[Lorentz 变换]] 使用保持 Minkowski 度量的伪正交群。
