...
- [[积分]] 和 [[多重积分]] 提供一维与多维积分规则；
- [[矩阵]] 的特征值、对角化和行列式控制多维二次型；
- [[概率]] 中的正态分布由 Gaussian 积分归一化；
- [[概率分布|概率分布]] 与 [[正态分布与常见分布|正态分布与常见分布]] 使用这里的积分结果确定正态密度的归一化常数；Gaussian 积分是积分方法，正态分布则是满足总概率为一的概率模型，二者不能直接等同；
- [[量子谐振子]] 的基态波函数是 Gaussian；
- [[Grassmann 变量]] 给出费米 Gaussian 积分；
- [[路径积分]] 把自由理论化为无限维 Gaussian 结构；
- [[Wick 旋转]] 把部分振荡积分转为衰减积分；
- [[生成泛函]] 通过源导数生成关联函数；
- [[传播子]] 是二次动力学算符的逆；
