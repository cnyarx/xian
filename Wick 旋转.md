...
- [[复数]] 和复平面轮廓变形提供解析延拓语言；
- [[Minkowski 时空]] 与 Euclidean 空间具有不同度量签名；
- [[路径积分]] 的实时权重经旋转变为 Euclidean 衰减权重；
- [[二维 Euclidean 场论]] 固定二维复坐标、面积元、Green 函数与反射正性的共同约定；
- [[Gaussian 积分]] 在 Euclidean 正定二次型下更易收敛；
- [[传播子]] 的解析延拓由极点和 $i\varepsilon$ 处方控制；
- [[有限温场论]] 把 Euclidean 时间紧致为长度 $\beta\hbar$ 的圆；
- [[世界面路径积分]] 常在 Euclidean Riemann 曲面上定义。
