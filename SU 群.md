...
- [[Lie 群]] 提供连续群与光滑结构；
- [[Lie 代数]] 描述 $SU(n)$ 的无穷小生成元；
- [[复数]] 提供复共轭、复相位和单位复数；
- [[矩阵]] 提供 Hermitian 共轭、逆矩阵、迹和行列式；
- [[内积空间]] 解释酉变换为何保持概率内积；
- [[SU(2) 与 SO(3) 的双覆盖]] 详细解释半角、覆盖核和半整数自旋；
- [[群表示]] 区分基本表示、伴随表示和其他不可约表示；
- [[旋量]] 与 $SU(2)\cong Spin(3)$ 的表示密切相关；
- [[自旋]] 使用 $SU(2)$ 表示分类三维旋转下的量子态；
- [[Yang-Mills 理论]] 常以 $SU(n)$ 作为规范群。
