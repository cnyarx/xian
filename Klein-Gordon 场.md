...
- [[作用量]] 与 [[经典场论]] 给出 KG 作用量的构造和场论框架；
- [[场的 Euler-Lagrange 方程]] 从作用量变分推出 $(\Box+m^2)\phi=0$；
- [[狭义相对论]] 与 [[Minkowski 时空]] 保证方程的 Lorentz 协变性；
- [[偏导数]] 与 [[积分]] 构建微分方程和 Fourier 解；
- [[Fourier 变换]] 提供平面波基和动量空间表示；
- [[量子算符]] 是二次量子化时 $\phi$ 和 $\pi$ 提升为算符的基础；
- [[Dirac 场]] 用旋量和 Gamma 矩阵构造自旋 $\frac12$ 场的一阶相对论方程；其每个分量进一步满足 Klein-Gordon 型二阶方程；
- [[场的正则量子化]] 把经典 KG 场提升为量子场；
- [[传播子]] 是 KG 场的 Green 函数，描述粒子在两点间传播的振幅；
- [[生成泛函]] 通过 KG 场引入路径积分和相关函数；
- [[复坐标与全纯分解]] 将二维无质量 KG 方程改写为 $\partial\bar\partial X=0$，并区分局部两扇区解与周期、现实性、动量和绕数等全局约束；
- [[弦的模展开]] 中的 $X^\mu$ 节是二维自由 KG 场的直接应用；
- [[二维自由玻色子]] 是在弦世界面上定义的二维 $m=0$ 标量场 CFT；
- [[能动张量]] 从 KG 场的时空平移不变性得到能量和动量密度；
- [[Noether 定理]] 从 $U(1)$ 对称性推导复 KG 场的守恒流。
