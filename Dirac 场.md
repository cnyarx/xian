...
- [[Klein-Gordon 场]] 给出 Dirac 方程平方后满足的相对论质量壳方程；
- [[费米子]] 解释反对易关系、Pauli 原理和费米统计；
- [[旋量]] 说明 $\psi$ 在 Lorentz 变换下为何不同于普通向量；
- [[Gamma 矩阵]] 与 [[Clifford 代数]] 构造 Dirac 算符；
- [[作用量]] 与 [[场的 Euler-Lagrange 方程]] 推出 Dirac 方程和共轭方程；
- [[Minkowski 时空]] 固定号差、质量壳与 Lorentz 协变性；
- [[量子算符]] 是把经典 Grassmann 场提升为算符值分布的基础；
- [[场的正则量子化]] 建立场和模式反对易关系；
- [[传播子]] 中的 Dirac 传播子是 Dirac 算符的 Green 函数；
- [[Wick 定理]] 对费米场交换顺序时产生额外负号；
- [[手征反常]] 说明经典轴对称怎样在量子层面破坏；
- [[规范协变导数]] 将本页的电磁最小耦合推广到一般非 Abelian 规范群，并解释不同符号约定的对应；
- [[二维自由费米子]] 是世界面共形场论的费米基础；
- [[世界面费米子]] 与 [[RNS 超弦理论]] 把二维旋量和目标时空旋量谱联系起来。
