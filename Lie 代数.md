...
- [[向量空间]] 是 Lie 代数的底层线性结构；
- [[对易关系]] 的矩阵或算符对易子给出最重要的 Lie 括号实例；
- [[群表示]] 为 Lie 代数表示提供向量空间和生成元矩阵背景；
- [[Lie 群]] 的单位元切空间带自然 Lie 代数结构；
- [[Lie 群与 Lie 代数的对应]] 解释指数映射和局部群乘法；
- [[SO 群]] 与 [[SU 群]] 具有重要矩阵 Lie 代数；
- [[角动量]] 满足 $\mathfrak{su}(2)$ 型对易关系；
- [[Yang-Mills 理论]] 使用 Lie 代数值规范场和非 Abel 场强；
- [[beta 函数]] 中 QCD 的一圈系数使用 $C_A$、$T_F$ 和生成元迹归一化。
