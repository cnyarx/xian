...
它是从算符微扰论通向 [[Feynman 图]] 的桥梁。图中的内线对应收缩，顶点来自相互作用，外部未收缩场对应外线。
...
- [[时序乘积]] 提供 Wick 定理左边的算符排序；
- [[传播子]] 是每一条自由场收缩的数值；
- [[产生算符]] 与 [[湮灭算符]] 的代数给出算符证明；
- [[Fock 空间]] 和自由真空决定正规序定义；
- [[Dirac 场]] 展示费米收缩、旋量指标和置换负号；
- [[微扰论]] 的 Dyson 展开产生需要 Wick 分解的时序乘积；
- [[生成泛函]] 从 Gaussian 源泛函推导同一配对规则；
- [[Feynman 图]] 是 Wick 收缩的图形记账；
- [[圈展开]] 按图的拓扑和闭环数组织量子修正；
- [[正规序]] 系统处理产生、湮灭顺序与复合算符；
- [[算符乘积展开]] 在短距离极限组织 Wick 收缩产生的奇异项；
- [[二维自由玻色子]] 用具体 Gaussian 收缩推导导数二点函数、分点正规序以及 $T(z)T(w)$ 的奇异项；
- [[弦散射振幅]] 使用世界面自由场 Wick 收缩计算顶点算符相关函数。
