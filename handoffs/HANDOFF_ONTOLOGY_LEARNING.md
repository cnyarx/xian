...
- `微扰论.md` 删除陈旧 `[[弦理论/物理学/重整化]]` 断链，改回 `正规化与重整化` 并新增 `重整化群` 链接；
- `对数函数.md` 删除两处“对应独立文件尚未建立”的旧说明，替换为真实 `[[弦理论/物理学/重整化群]]` 链接；
...
- `Grassmann 变量.md` 的 `used_by` 和关系区中 `[[弦理论/物理学/鬼场]]` 旧链接改为正式路径；
...
- `能动张量.md` 和 `度量.md` 原 `[[弦理论/物理学/共形反常]]` 断链改为正式 Weyl 页的别名链接；
...
- 后继 `[[弦理论/物理学/二维 Euclidean 场论]]` 已创建，反常消除页导航、`used_by` 和正文边界均已改为真实双向链接。
...
- 反常消除页导航改为真实 `[[弦理论/物理学/二维 Euclidean 场论]]`，补 `used_by`，边界文本同步；
- Wick 旋转页修复 `[[弦理论/物理学/Euclidean 场论]]`→`[[弦理论/物理学/二维 Euclidean 场论]]` 断链，补正文关系；
...
3. **链接改写**：全库 `[[数学/X]]`、`[[物理学/X]]`、`[[计算机学/X]]` 路径式链接（约 165 处，分布在图谱层内部、`大模型/` 4 个文件和两篇根目录文档）统一改写为纯文件名链接 `[[X]]`。三篇根目录文档的 71 处入链本就是纯文件名形式，移动与否均可解析，未做改动。

### 新约定

- 根目录 `*.md`（带 `跨学科` tag）= 跨学科图谱层；`弦理论/`、`大模型学习教程/`、`大模型/` = 正式命名空间，本次零结构改动。
- 本交接文档此前章节中出现的 `数学/…`、`物理学/…`、`计算机学/…` 路径均为历史记录，现已失效；对应文件在仓库根目录。
- 路线图中 `[[弦理论/物理学/世界面]]`、`[[弦理论/物理学/RNS 超弦理论]]`、`[[弦理论/物理学/临界维数]]`、`[[弦理论/数学/中心荷]]` 等 23 处链接是**规划中正式节点的预链接**（重组前即未解析），刻意保留不改写，以免把"计划中"误标为"已完成"。**后续创建这些正式节点时，必须按 Weyl 反常模式处理同名根目录概览页：内容并入正式页（不丢失、不重复）、删除概览页、重定向入链，禁止同名两页并存。**
