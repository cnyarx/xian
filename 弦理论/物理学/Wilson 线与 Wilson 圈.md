...
[[弦理论/物理学/规范协变导数]] 描述无穷小邻域内怎样比较物质场的内部方向，[[弦理论/物理学/Yang-Mills 理论]] 用对易子 $[D_\mu,D_\nu]$ 定义局部场强 $F_{\mu\nu}$。但局部曲率不能完整回答以下问题：
...
这里使用电磁约定 $D_\mu=\partial_\mu-iqA_\mu$；若采用 [[弦理论/物理学/Dirac 场]] 中的 $D_\mu=\partial_\mu+iqA_\mu$，指数符号相反。

即使局部经过区域满足 $F_{\mu\nu}=0$，若路径绕过不可达磁通区域，闭合相位仍可非平凡。这就是 Aharonov-Bohm 效应揭示的全局 holonomy：局部场强为零不等于所有闭合 Wilson 圈都平凡。

## 与曲率和 holonomy 的关系

[[弦理论/数学/曲率]] 说明曲率测量无穷小平行移动的不对易。设一族闭合回路 $C_\ell=\partial\Sigma_\ell$ 的线性尺度为 $\ell$，并定义有向面积双向量
...
- [[弦理论/物理学/规范协变导数]] 是 Wilson 线的无穷小起点；
- [[弦理论/物理学/Yang-Mills 理论]] 定义规范势、场强和表示约定，并说明局部公式不能替代全局 holonomy；
- [[弦理论/数学/联络]] 用平行移动解释 Wilson 线的几何含义；
- [[弦理论/数学/曲率]] 给出小回路 holonomy 的局部控制量；
- [[弦理论/数学/群表示]] 说明不同表示中的 Wilson 算符为何不同；
- [[弦理论/数学/Lie 群与 Lie 代数的对应]] 说明 Wilson 线还依赖全局规范群，而不只依赖 Lie 代数；
- [[弦理论/物理学/Maxwell 场量子化]] 说明紧致空间中的 Wilson 线和磁通自由度不包含在局部平面波计数中；
- [[弦理论/弦理论完整学习路线图]] 将本节点放在规范协变导数之后、规范固定之前。
